...
export const LoadingPlan: React.FC<LoadingPlanProps> = ({
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const {
        ...
        `).eq('is_received', true).eq('current_transitaire', transitaire).order('created_at', {
        ascending: false
      });
      ...
      if (type === 'container') {
        ...
        const {
          data: bookingsData,
          error: bookingsError
        } = await supabase.from('groupage_bookings').select(`
            *,
            orders!inner(
              *,
              order_products(
  ...
  const canAddOrder = (order: Order) => {
    if (type === 'container' && container) {
      ...
      return {
        ...
        reason: !transitaireMatch ? 'Transitaire différent' : !dangerousAllowed ? 'Produits dangereux non autorisés' : ''
      };
      ...
      return {
        ...
        reason: !transitaireMatch ? 'Transitaire différent' : !dangerousAllowed ? 'Produits dangereux non autorisés' : !statusOk ? 'Groupage non disponible' : ''
      };
    }
    return {
      canAdd: false,
  ...
  const handleAddOrder = async (data: any) => {
    ...
    try {
      if (type === 'container' && container) {
        ...
        toast({
          title: "Succès",
          description: "Commande ajoutée au conteneur"
        });
      } else if (type === 'groupage' && groupage) {
        // Ajouter au groupage
        ...
        const {
          error: bookingError
        } = await supabase.from('groupage_bookings').insert([{
        ...
        const {
          error: updateError
        } = await supabase.from('groupages').update({
        ...
        toast({
          title: "Succès",
          description: "Commande réservée dans le groupage"
        });
      }
  ...
  const handleRemoveOrder = async (orderId: string) => {
    try {
      if (type === 'container') {
        ...
        toast({
          title: "Succès",
          description: "Commande retirée du conteneur"
        });
        ...
        const {
          error: deleteError
        } = await supabase.from('groupage_bookings').delete().eq('id', booking.id);
  ...
  const handleConfirmBooking = async (bookingId: string, confirm: boolean) => {
    try {
      const {
        error
      } = await supabase.from('groupage_bookings').update({
      ...
      toast({
        title: "Succès",
        description: confirm ? "Réservation confirmée" : "Réservation annulée"
      });
      fetchOrders();
    } catch (error) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 justify-items-center">
                <Card className="w-full max-w-xs mx-auto">
                  <CardContent className="p-4 text-center">
                    ...
                    {(container || groupage) && <div className="text-xs text-muted-foreground">
            ...
            <div className="lg:col-span-2">
              <Card>
                ...
                <CardContent>
                  {loading ? <div>Chargement...</div> : type === 'container' ? assignedOrders.length === 0 ? <p className="text-muted-foreground text-center py-4">Aucune commande assignée</p> : <div className="space-y-3">
                        {assignedOrders.map(order => <div key={order.id} className="flex items-center justify-between p-3 border rounded-lg">
                            <div className="flex-1 cursor-pointer" onClick={() => window.open(`/orders/${order.id}`, '_blank')}>
                              ...
                              {order.transitaire_entry_number && <div className="text-sm text-blue-600 font-medium">
                             ...
                             <div className="flex gap-2 items-center">
                               <div className="flex flex-col items-center gap-1">
                                 ...
                                 <Switch id={`empotage-${order.id}`} checked={empotageConfirmed[order.id] || false} onCheckedChange={checked => setEmpotageConfirmed(prev => ({
                          ...prev,
                          [order.id]: checked
                        }))} />
                               </div>
                               
                               {!empotageConfirmed[order.id] && <Button variant="outline" size="sm" onClick={() => handleRemoveOrder(order.id)}>
                                   <Minus className="h-4 w-4" />
                                 </Button>}
                             </div>
                          </div>)}
                      </div> : groupageBookings.length === 0 ? <p className="text-muted-foreground text-center py-4">Aucune réservation</p> : <div className="space-y-3">
                        {groupageBookings.map(booking => <div key={booking.id} className="flex items-center justify-between p-3 border rounded-lg">
                            <div className="flex-1 cursor-pointer" onClick={() => window.open(`/orders/${booking.order_id}`, '_blank')}>
                              ...
                              {booking.orders?.transitaire_entry_number && <div className="text-sm text-blue-600 font-medium">
                             ...
                             <div className="flex gap-2 items-center">
                               <div className="flex flex-col items-center gap-1">
                                 ...
                                 <Switch id={`empotage-${booking.order_id}`} checked={empotageConfirmed[booking.order_id] || false} onCheckedChange={checked => setEmpotageConfirmed(prev => ({
                          ...prev,
                          [booking.order_id]: checked
                        }))} />
                               </div>
                               {booking.booking_status === 'pending' && <>
                                   <Button variant="outline" size="sm" onClick={() => handleConfirmBooking(booking.id, true)}>
                                     <Check className="h-4 w-4" />
                                   </Button>
                                   <Button variant="outline" size="sm" onClick={() => handleConfirmBooking(booking.id, false)}>
                                     <X className="h-4 w-4" />
                                   </Button>
                                 </>}
                                {!empotageConfirmed[booking.order_id] && <Button variant="outline" size="sm" onClick={() => handleRemoveOrder(booking.order_id)}>
                                    <Minus className="h-4 w-4" />
                                  </Button>}
                             </div>
                          </div>)}
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  {filteredAvailableOrders.length === 0 ? <p className="text-muted-foreground text-center py-4">{searchTerm ? 'Aucune commande trouvée' : 'Aucune commande disponible'}</p> : <div className="space-y-2 max-h-96 overflow-y-auto">
                    ...
                    return <div key={order.id} className={`p-2 border rounded cursor-pointer transition-colors ${canAdd ? 'hover:bg-muted' : 'bg-muted/50 cursor-not-allowed'}`} onClick={() => canAdd && (setSelectedOrder(order), setIsAddOrderOpen(true))}>
                            <div className="text-sm font-medium">{order.order_number}</div>
                            <div className="text-xs text-muted-foreground">{order.supplier}</div>
                            {!canAdd && <div className="text-xs text-red-500 mt-1">{reason}</div>}
                          </div>;
                  })}
                    </div>}
                </CardContent>
          ...
          <form onSubmit={bookingForm.handleSubmit(handleAddOrder)} className="space-y-4">
            {type === 'groupage' && <>
                <div>
                  ...
                  <Input id="palettes_booked" type="number" {...bookingForm.register('palettes_booked', {
                required: true
                ...
                <div>
                  ...
                  <Input id="weight_booked" type="number" step="0.1" {...bookingForm.register('weight_booked', {
                required: true
                ...
                <div>
                  ...
                  <Input id="volume_booked" type="number" step="0.1" {...bookingForm.register('volume_booked', {
                required: true
